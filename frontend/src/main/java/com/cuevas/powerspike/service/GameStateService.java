...
public class GameStateService {
    ...
    public GameStateService(LocalSessionStore sessionStore) {
        this.sessionStore = sessionStore;
    }

    /**
     * Se ejecuta cuando Spring crea el bean (una vez, al arrancar la app).
     * Busca la sesión guardada en el archivo local (data/session.json) y la
     * restaura en las properties. NO usa Platform.runLater porque acá todavía
     * no está corriendo el FX thread.
     */
    @PostConstruct
    public void restoreSession() {
        sessionStore.load().ifPresent(s -> {
            myPuuid.set(s.puuid());
            myGameName.set(s.gameName());
            myTagLine.set(s.tagLine());
            profileIconId.set(s.profileIconId());
            summonerLevel.set(s.summonerLevel());
        });
    }

    /**
     * Guarda la sesión activa en el archivo local (persiste entre reinicios).
    ...
    public void saveSession(String puuid, String gameName, String tagLine, Long profileIconId, Long summonerLevel) {
        sessionStore.save(new LocalSessionStore.SessionData(
                puuid, gameName, tagLine,
                profileIconId != null ? profileIconId : 0,
                summonerLevel != null ? summonerLevel : 0));
        myPuuid.set(puuid);
    ...
    /**
     * Borra la sesión del archivo local y limpia las properties.
     */
