...
public class PromptBuilder {
    ...
    public String buildLiveClientMatchupPrompt(LiveClientAllDataDTO data, 
        ...
        return """
                ...
                Con tu conocimiento general del juego (sin inventar números exactos de cooldowns o daño), analizá:
    ...
    public String buildDeathPrompt(LiveClientAllDataDTO data, LiveClientEventDTO death,
        ...
        double csPerMin = myPlayer != null && death.EventTime() > 60
                ? (myPlayer.scores() != null ? myPlayer.scores().creepScore() : 0) / (death.EventTime() / 60.0)
                : 0;
        ...
        return """
                ...
                fightType,
                visionText,
                killerComparison != null && !killerComparison.isBlank() ? killerComparison : "",
    ...
    public String buildObjectiveSpawnPrompt(LiveClientAllDataDTO data, String objective, double spawnTime,
                                            double currentGameTime, int myKills, int enemyKills,
                                            int advantage, long deadAllies) {
        int spawnMin = (int)(spawnTime / 60);
        int spawnSec = (int)(spawnTime % 60);
        int curMin = (int)(currentGameTime / 60);
        int curSec = (int)(currentGameTime % 60);
        ...
        return """
                %s va a aparecer en ~30 segundos (hora del spawn: %d:%02d).
                Hora actual de la partida: %d:%02d.
                ...
                Decime si vale la pena pelearlo o no y qué hacer ahora (máximo 3 líneas).
                """.formatted(
                objective, spawnMin, spawnSec, curMin, curSec,
                myChamp, myRole,
                myStats,
                myItems,
                myKills, enemyKills,
    ...
    public String buildGameEndPrompt(AnalysisContext ctx) {
        ...
        int myTeamKills = "ORDER".equals(myTeam) ? totalKillsBlue : totalKillsRed;
        int myKills = myPlayer != null && myPlayer.scores() != null ? myPlayer.scores().kills() : 0;
        int myAssists = myPlayer != null && myPlayer.scores() != null ? myPlayer.scores().assists() : 0;
        int myCs = myPlayer != null && myPlayer.scores() != null ? myPlayer.scores().creepScore() : 0;
        double kp = myTeamKills > 0 ? (myKills + myAssists) * 100.0 / myTeamKills : 0;
        double csPerMin = gameMinutes > 0 ? myCs / gameMinutes : 0;

        return """
                La partida terminó (%.1f minutos).

                Tu jugador: %s (%s).
                Stats finales: %s (%.1f CS/min, %d%% KP)
                Build: %s
